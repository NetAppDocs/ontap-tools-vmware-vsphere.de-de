...
|===
Sie sollten das Skript aus der Kopie der VSC-Instanz installieren, die beim vCenter Server (ELM) registriert ist, der die virtuelle Maschine verwaltet. Wenn Ihre Umgebung mehrere vCenter-Server umfasst, sollten Sie die Instanz auswählen, die die virtuelle Maschine enthält, für die Sie die Werte für das Speicher-Timeout festlegen möchten.
